...
export class PreMissionApi {
  ...
  create(request: AssessmentCreateRequest) { return this.http.post<unknown>(this.url, { regionId: request.regionId, plannedStart: request.plannedStart, plannedEnd: request.plannedEnd, assetIds: request.scopeAssetIds }).pipe(map((response) => normalizeAssessment(unwrapApiData(response)))); }
...
const normalizeAssessment = (value: unknown): PreMissionAssessment => {
  ...
  const derivedCheck = (ready: boolean) => ({ status: ready ? 'PASS' : 'PENDING', reason: null, evaluatedAt: null });
  return {
    id: stringOf(x['id']), assessmentCode: stringOf(x['assessmentCode'] ?? x['code'], 'ASSESSMENT'),
    regionName: stringOf(x['regionName'] ?? x['regionId'], 'Unknown region'),
    lineName: x['lineName'] == null ? null : stringOf(x['lineName']), assetCount: Number(x['assetCount'] ?? scopeAssetIds.length),
    plannedStart: stringOf(x['plannedStart']), plannedEnd: stringOf(x['plannedEnd']),
    ...
    uav: x['uav'] ? checkOf(x['uav']) : derivedCheck(arrayOf(x['droneCandidates']).length > 0),
    technical: x['technical'] ? checkOf(x['technical']) : derivedCheck(status === 'READY'), status,
    validUntil: x['validUntil'] == null ? null : stringOf(x['validUntil']), createdBy: x['createdBy'] == null ? null : stringOf(x['createdBy']),
    updatedAt: x['updatedAt'] == null ? null : stringOf(x['updatedAt']), consumedMissionId: x['consumedMissionId'] == null ? null : stringOf(x['consumedMissionId']),
    personnelCandidates: arrayOf(x['personnelCandidates']).map(objectOf) as never, uavCandidates: arrayOf(x['uavCandidates']).map(objectOf) as never,
    technicalMetrics: arrayOf(x['technicalMetrics']).map(objectOf) as never, scopeAssetIds, scopeGeometry: x['scopeGeometry']
  };
};
const normalizePage = (value: unknown, page: number, pageSize: number): AssessmentPage => {
  const x = objectOf(value); const rawItems = Array.isArray(value) ? value : (x['items'] ?? x['records']);
  const items = arrayOf(rawItems).map(normalizeAssessment); const totalCount = Number(x['totalCount'] ?? items.length);
  return { items, page: Number(x['page'] ?? page), pageSize: Number(x['pageSize'] ?? pageSize), totalCount, totalPages: Number(x['totalPages'] ?? Math.max(1, Math.ceil(totalCount / pageSize))) };
};
